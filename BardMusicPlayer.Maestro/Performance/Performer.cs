...
namespace BardMusicPlayer.Maestro.Performance
{
    public sealed class Performer
    {
        ...
        private Timer _startDelayTimer { get; } = new();
        ...
        public string SongName
        {
            get
            {
                ...
                return _sequencer.LoadedBmpSong.DisplayedTitle.Length < 2
                    ? //title with 1 char makes no sence for me
        ...
        public string TrackInstrument
        {
            get
            {
                ...
                var classicConfig = (ClassicProcessorConfig)_sequencer.LoadedBmpSong.TrackContainers[TrackNumber - 1]
        ...
        public void OpenInstrument()
        {
            ...
            if (UsesDalamud)
            {
                DalamudBridge.DalamudBridge.Instance.ActionToQueue(new DalamudBridgeCommandStruct
                {
                    messageType = DalamudBridge.Helper.Dalamud.MessageType.Instrument, game = game,
                    IntData = Instrument.Parse(TrackInstrument).Index
                });
            }
            else
            {
        ...
        public async Task<int> ReplaceInstrument()
        {
            ...
            if (!game.InstrumentHeld.Equals(Instrument.None))
            {
                ...
                if (UsesDalamud)
                    DalamudBridge.DalamudBridge.Instance.ActionToQueue(new DalamudBridgeCommandStruct
                    {
                        messageType = DalamudBridge.Helper.Dalamud.MessageType.Instrument, game = game, IntData = 0
                    });
                else
        ...
        public void EnsembleAccept()
        {
            ...
            if (UsesDalamud)
            {
                DalamudBridge.DalamudBridge.Instance.ActionToQueue(new DalamudBridgeCommandStruct
                {
                    messageType = DalamudBridge.Helper.Dalamud.MessageType.AcceptReply, game = game, BoolData = true
                });
                return;
            }
