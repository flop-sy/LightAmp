...
internal class Unzip : IDisposable
{
    ...
    private Entry GetEntry(string fileName)
    {
        ...
        if (entry == null) throw new FileNotFoundException("File not found in the archive: " + fileName);
    ...
    public void Extract(Entry entry, Stream outputStream)
    {
        ...
        if (entry.Deflated) inputStream = new DeflateStream(Stream, CompressionMode.Decompress, true);
    ...
    {
        if (Stream.Length < 22) yield break;
        ...
        {
            if (Stream.Position <= 5) yield break;
        ...
        {
            if (Reader.ReadInt32() != EntrySignature) continue;
