...
internal static class ConfigParser
{
    ...
    /// <returns></returns>
    internal static Dictionary<long, ConfigContainer> ReadConfigs(this TrackChunk trackChunk, int trackNumber,
        BmpSong song)
    {
        ...
        {
            BmpLog.I(BmpLog.Source.Transmogrify,
                "Skipping track " + trackNumber + " as the track title contains \"Ignore\"");
            return configContainers;
        }
        ...
        for (var groupCounter = 0; groupCounter < groups.Length; groupCounter++)
        {
                ...
                switch (subfields[0])
                {
                    case "vst" when subfields.Length < 2:
                        BmpLog.W(BmpLog.Source.Transmogrify,
                            "Skipping VST on track " + trackNumber +
                            " due to the configuration not specifying a tone.");
                        continue;
                    case "vst":
                        var manualToneConfig = (VSTProcessorConfig)(configContainer.ProcessorConfig =
                        ...
                        if (manualToneConfig.InstrumentTone.Equals(InstrumentTone.None))
                        {
                            BmpLog.W(BmpLog.Source.Transmogrify,
                                "Skipping VST on track " + trackNumber +
                                " due to the configuration specifying an invalid tone.");
                            continue;
                        }
                        ...
                        if (subfields.Length > 2)
                        {
                            ...
                            foreach (var shift in shifts)
                            {
                                ...
                                if (!toneIndexAndOctaveRange.Groups[1].Success)
                                {
                                    BmpLog.W(BmpLog.Source.Transmogrify,
                                        "Skipping invalid VST octave setting \"" + shift + "\" on track " +
                                        trackNumber + " because \"" + toneIndexAndOctaveRange.Groups[1].Value +
                                        "\" is not a valid tone number");
                                    continue;
                                }
                        ...
                        BmpLog.I(BmpLog.Source.Transmogrify,
                            "Found VST Config Group with on track " + manualToneConfig.Track + " ;bards=" +
                            manualToneConfig.PlayerCount + ";include=" +
                            string.Join(",", manualToneConfig.IncludedTracks));
                        configContainers.Add(groupCounter, configContainer);
                        continue;

                    case "lyrics":
                        var lyricConfig = (LyricProcessorConfig)(configContainer.ProcessorConfig =
                            new LyricProcessorConfig { Track = trackNumber });
                        ParseAdditionalOptions(trackNumber, lyricConfig, song, fields);
                        BmpLog.I(BmpLog.Source.Transmogrify,
                            "Found Lyric Config on track " + lyricConfig.Track + " ;bards=" +
                            lyricConfig.PlayerCount + ";include=" + string.Join(",", lyricConfig.IncludedTracks));
                        configContainers.Add(groupCounter, configContainer);
                        continue;
                }
            }
            ...
            else
            {
                ...
                BmpLog.I(BmpLog.Source.Transmogrify,
                    ...
                    classicConfig.OctaveRange.Name + " on track " + classicConfig.Track + " ;bards=" +
                    classicConfig.PlayerCount + ";include=" + string.Join(",", classicConfig.IncludedTracks));
                configContainers.Add(groupCounter, configContainer);
            }
        }
    ...
    private static void ParseAdditionalOptions(int trackNumber, IProcessorConfig processorConfig, BmpSong song,
        ...
        for (var fieldCounter = 1; fieldCounter < fields.Count; fieldCounter++)
            if (fields[fieldCounter].StartsWith("include=", StringComparison.Ordinal))
            {
                ...
                foreach (var trackToMerge in tracksToMerge)
                    if (int.TryParse(trackToMerge, out var value) && value != trackNumber && value > -1 &&
