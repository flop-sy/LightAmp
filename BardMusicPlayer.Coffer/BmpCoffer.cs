...
public sealed class BmpCoffer : IDisposable
{
    ...
    internal static BmpCoffer CreateInstance(string dbPath)
    {
        ...
        mapper.RegisterType(static tempoMap => SerializeTempoMap(tempoMap),
            static bson => DeserializeTempoMap(bson.AsBinary));
        mapper.RegisterType(static trackChunk => SerializeTrackChunk(trackChunk),
            static bson => DeserializeTrackChunk(bson.AsBinary));
    ...
    public void DeletePlaylist(IPlaylist songList)
    {
        ...
        try
        {
            if (dbList.Id != null) playlists.Delete(dbList.Id);
        }
        catch (LiteException e)
        {
            throw new BmpCofferException(e.Message, e);
