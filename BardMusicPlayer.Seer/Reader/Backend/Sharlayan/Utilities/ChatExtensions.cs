...
{
    private static readonly Regex Romans =
        new("(?<roman>\\b[IVXLCDM]+\\b)", RegexOptions.ExplicitCapture | RegexOptions.Compiled);

    private static readonly Regex Titles =
        new("(?<num>\\d+)(?<designator>\\w+)", RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture |
